...
parser.add_argument('--backprop_every',
                    type=int,
                    default=1,
...
parser.add_argument('--message_function',
                    type=str,
                    default="identity",
                    choices=["mlp", "identity"],
                    help='Type of message function')
parser.add_argument('--memory_updater',
                    type=str,
                    default="gru",
                    choices=["gru", "rnn"],
...
parser.add_argument('--memory_dim',
                    type=int,
                    default=172,
...
parser.add_argument('--randomize_features',
                    action='store_true',
...
nn_val_rand_sampler = RandEdgeSampler(new_node_val_data.sources,
                                      ...
                                      seed=1)
test_rand_sampler = RandEdgeSampler(full_data.sources,
                                    full_data.destinations,
                                    seed=2)
...
for i in range(args.n_runs):
    ...
    tgn = TGN(
        ...
        use_destination_embedding_in_message=args.
        use_destination_embedding_in_message,
    ...
    for epoch in range(NUM_EPOCH):
        ...
        logger.info('val auc: {}, new node val auc: {}'.format(
            val_auc, nn_val_auc))
        logger.info('val ap: {}, new node val ap: {}'.format(
            val_ap, nn_val_ap))
    ...
    logger.info('Test statistics: Old nodes -- auc: {}, ap: {}'.format(
        test_auc, test_ap))
    logger.info('Test statistics: New nodes -- auc: {}, ap: {}'.format(
        nn_test_auc, nn_test_ap))
